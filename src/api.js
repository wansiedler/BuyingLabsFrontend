...
const getHotels = async (cityCode, checkInDate, checkOutDate, adults) => {
    try {
        const c = `/api/hotels?cityCode=${cityCode}&checkInDate=${checkInDate}&checkOutDate=${checkOutDate}&adults=${adults}`
        console.log(c)
...
const getOffers = async (hotelId, adults) => {
    try {
        const c = `/api/offers?hotelId=${hotelId}&adults=${adults}`
        console.log(c)
        const response = await axios.get(c);
        const json = response.data;
